...
function Series() {
    ...
    return <div>
        <span className='pageTitle'>Searies</span>
        ...
        <div className='trending'>
            {
                content && content.map(({ id, poster_path, title, name, first_air_date, release_date, vote_average }) => {
                    return (
                        <SingleContent key={id} id={id} poster={poster_path} title={title || name} date={first_air_date || release_date} media_type='tv' vote_average={vote_average} />
                    )
                })
            }
        </div>
